...
export const DEFAULT_COPILOT_MODEL_ID = "claude-sonnet-5";
export const THINKING_LEVELS = ["auto", "low", "medium", "high", "xhigh", "max", "ultracode"] as const;
export const HARNESS_IDS = ["pi", "opencode", "codex", "claude", "copilot", "mock"] as const;
...
export function modelSupportedByHarness(id: string | undefined, harness: string): boolean {
  ...
  if (isCustomModelId(id) && !REGISTRY_BY_ID.has(id))
    return harness === "pi" || harness === "opencode" || harness === "copilot" || harness === "mock";
  if (harness === "pi" || harness === "opencode" || harness === "copilot" || harness === "mock")
    return Boolean(resolveModel(id));
